...
(function() {
    ...
    function getNJDOT() {
        getFeed(NJURLList,"json", function(result) {
            var resultObj = JSON.parse(result.responseText).Data.features;
            var i=0;
            var promises = [];
            while (i<resultObj.length) {
                if (resultObj[i].properties.CategoryName == "Incident" || resultObj[i].properties.CategoryName == "Construction" || resultObj[i].properties.CategoryName == "ScheduledConstruction" || resultObj[i].properties.CategoryName == "Detour") {
                    //console.log("NJ Data: " + i + " - " + resultObj[i].properties.CategoryName);
                    getFeed(NJURLDetail + resultObj[i].properties.EventID,"json", function(result2) {
                        var eventObj = JSON.parse(result2.responseText).Data;
                        var icon;
                        switch(eventObj[0].CategoryName) {
                            case "Incident":
                                icon = "Incident";
                                //console.log(eventObj[0].markerId);
                                break;
                            case "Detour":
                                icon = "Incident";
                                break;

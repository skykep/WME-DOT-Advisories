...
let AKFeed = [], AZFeed = [], CTFeed = [], DEFeed = [], FLFeed = [], GAFeed = [], ILFeed = [], INFeed = [], LAFeed = [], MDFeed = [], MIFeed = [], MNFeed = [], NCFeed = [], NJFeed = [], NVFeed = [], NYFeed = [], OHFeed = [], ORFeed = [], PAFeed = [], TXFeed = [], VAFeed = [], WAFeed = [], WIFeed = [], WV;
let settings;
let mapBounds;
let state, stateLength, advisories;
const updateMessage = "&#9658; Bug Fix (Interference with US Government Boundaries";
...
(function () {
    ...
    const config = { //Configuration data for each state
        ...
        MN: {
            ...
            filter(index) {
                let filtertext = ['resultObj[i].features[0].properties.icon.url == "/images/tg_closure_routine.svg" || resultObj[i].features[0].properties.icon.url == "/images/tg_crash_routine.svg" || (resultObj[i].features[0].properties.icon.url == "/images/tg_warning_routine.svg" && resultObj[i].tooltip.toUpperCase().includes("RAMP CLOSE"))'];
                return (filtertext[index]);
            },
            scheme(obj, index) {
                let advType = "";

...
const updateMessage = "&#9658; Added OR Incidents";
...
(function () {
    ...
    function getAdvisories(state, stateAbv, type) {
        ...
        for (let j = 0; j < state.URL.length; j++) {
            ...
            console.log(new Date().toLocaleString() + "promises pushed");
        }
        console.log(new Date().toLocaleString() + "loop done");
        Promise.all(thesepromises).then(function () {
            setTimeout(function () { promiseWorker(stateAbv, type) }, 500);
            console.log(new Date().toLocaleString() + "calling the main promise");
        })
                    ...
                    case 1:
                        promisesOR.push(new Promise((resolve, reject) => {
                            advisoriesOR.push({
                                ...
                                keyword: ['ROADWORK'], //keywords for roadwork/construction
                                desc: obj.attributes.comments + " - " + obj.attributes.beginMarker + " to " + obj.attributes.endMarker,
